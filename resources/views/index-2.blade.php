...
@section('content')
  @if(getFrontSection('hero_active') == 1)
    <section class="bg-[#001] text-white relative isolate mx-3 lg:mx-7 hero rounded-[1.5rem]" id="home">
      <div
      ...
      <div class="container relative overflow-hidden rounded-3xl">
        ...
        <div class="mx-auto max-w-5xl py-16 text-center">
          <div
            class="relative overflow-hidden inline-flex items-center rounded-3xl border border-slate-200 px-4 py-1 space-x-3 bg-dark">
          ...
          <h1 class="mt-8 mb-5 text-5xl font-semibold lg:text-6xl">
          ...
          <p class="mb-8 text-md">
            {{getFrontSection('hero_description')}}
          </p>
    ...
    <section class="py-8">
      <div class="container">
        <div class="mx-auto mb-12 max-w-xl text-center">
          <div
            class="relative inline-flex overflow-hidden items-center rounded-full bg-green/60 text-slate-900 px-3 py-1 text-md font-bold">
            <span class="mr-2 inline-block text-md">🤩</span>
          ...
          <h2 class="my-3 text-3xl font-medium text-slate-700">
        ...
        <div class="lg:flex lg:flex-nowrap lg:space-x-10">
          <div class="lg:w-[43%] flex">
            <ul class="my-auto nav nav-tabs space-y-2" role="tablist">
              @foreach($howItWorks as $work)
                <li class="nav-item" role="presentation">
                  <button aria-controls="rewards-income"
                          aria-selected="false"
                          class="relative flex w-full border-b border-slate-100 p-6 text-left transition-all nav-link nav-link--style-2 group {{$loop->first ? 'active' : ''}}"
                          data-bs-target="#{{Str::slug($work->title)}}-panel"
                          data-bs-toggle="tab"
                          id="{{Str::slug($work->title)}}-tab"
                          role="tab"
                          type="button">
                <span
                  class="mr-4 flex-shrink-0 rounded-xl px-4 py-2 text-white transition-all bg-dark group-hover:scale-125 [&.active]:scale-125">{{ $loop->index + 1 }}</span>
                    <div>
                      <span class="mt-1 mb-2 block text-xl font-medium group-hover:text-dark">{{$work->title}}</span>
                      <div class="hidden nav-link-content">
          ...
          <div class="flex items-center justify-center overflow-hidden lg:w-[57%] lg:overflow-visible">
            <div class="flex-1 p-4 tab-content">
              @foreach($howItWorks as $work)
                <div aria-labelledby="{{Str::slug($work->title)}}-tab"
                     class="relative tab-pane fade {{$loop->first ? 'active' : ''}}"
                     id="{{Str::slug($work->title)}}-panel"
                     role="tabpanel">
        ...
        <div class="mx-auto px-6 py-8">
          ...
          <div class="relative rounded-2xl bg-white px-5 py-3 lg:py-5 tab-content">
            <div class="tab-pane lg:p-3 fade show active" id="pricing-monthly" tabindex="0">
              <div class="-mx-6 mt-16 grid gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
